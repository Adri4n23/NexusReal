...
import { MessageCircle, MapPin, User, Home, Map, Share2, AlertCircle, Loader2, Tag, CalendarDays, Bed, Bath, Ruler, ArrowLeft, ArrowRight, DollarSign, FileText, Users, Send, Info, Building2 } from 'lucide-react';
...
function DetallePropiedad({ session, onNotificar }) {
  ...
  // Función para compartir ficha (WhatsApp)
  ...
  if (!propiedad) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-4">
        ...
        <div className="text-xs text-slate-400 mt-2">ID buscado: {id}</div>
        <button onClick={() => navigate('/')} className="mt-6 bg-blue-600 text-white px-6 py-3 rounded-xl hover:bg-blue-700 transition-colors">
          Volver al inicio
        </button>
      </div>
